...
選好層、開始寫斷言時，定位元素**優先用使用者／螢幕閱讀器感知得到的特徵**，順序：

1. **role + 無障礙名稱**——`getByRole('button', { name: '增加數量' })`、或 `find('button[aria-label="增加數量"]')`。role 是「這是什麼種類」（button／link／textbox…，多由標籤隱含），name 是它的無障礙名稱（來自 `aria-label`，沒有時退回可見文字）。
2. **label／可見文字**——`getByText('前往結帳')`、`find('button[aria-label="移除"]')`。
3. **`data-testid` 專屬標記**——前兩者都無合適特徵時的逃生門。

**不要**用 class 或 DOM 巢狀結構定位（那是樣式／實作細節，一重構測試就碎）。

某互動元素抓不到時，**優先回頭給它補 `aria-label`**（測試好抓 + a11y 更好），而不是在測試裡 `findAll` 全部再 `filter` 文字。`filter` 只保留給「本來就要一整組」的斷言（如列出畫面上所有連結的 `to`），不要拿來當「選不到單一元素」的替代手段。
