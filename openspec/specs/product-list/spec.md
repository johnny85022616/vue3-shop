...
### Requirement: 顯示商品列表
系統 SHALL 從 DummyJSON API 取得商品並以卡片形式顯示，每張卡片包含商品圖片、名稱、價格與評分。載入中狀態 SHALL 以商品資料本身是否回傳為準，不受分類清單載入狀態影響。
...
- **WHEN** 分類清單已載入完成，商品資料仍在載入中
- **THEN** 系統維持顯示載入中狀態，不顯示「找不到符合的商品」提示
